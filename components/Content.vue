    ...
    <aside class="sidebar">
      <div class="sidebar-content">
        ...
        <div class="related-news" v-if="relatedNews && relatedNews.length">
          <h3 class="sidebar-title">{{title}}ที่เกี่ยวข้อง</h3>
          ...
          <div class="related-articles">
            ...
            >
              ...
              <div class="related-content">
                <h4 class="related-title">{{ relatedArticle.name }}</h4>
          ...
          <div class="popular-articles">
            ...
            >
              ...
              <div class="popular-content">
                <h4 class="popular-title">{{ popularArticle.name }}</h4>
        ...
        <div class="latest-news" v-if="latestNews && latestNews.length">
          <h3 class="sidebar-title font-kanit">{{title}}ล่าสุด</h3>
          ...
          <div class="latest-articles font-kanit">
            ...
            >
              <h4 class="latest-title">{{ latestArticle.name }}</h4>
...
const {
  ...
  let query = $supabase
    .from(props.slug)
    .select("id, name, cover, created_at, views, category")
    .neq("id", id);
...
const navigateToArticle = (articleId) => {
  router.push(`/${props.slug}/${articleId}`);
};

// Social share functions
const shareToFacebook = () => {
  const url = encodeURIComponent(window.location.href);
  const title = encodeURIComponent(article.value.name);
  window.open(`https://www.facebook.com/sharer/sharer.php?u=${url}&quote=${name}`, '_blank');
};

const shareToTwitter = () => {
  const url = encodeURIComponent(window.location.href);
  const name = encodeURIComponent(article.value.name);
  window.open(`https://twitter.com/intent/tweet?url=${url}&text=${name}`, '_blank');
};

const shareToLine = () => {
  const url = encodeURIComponent(window.location.href);
  const name = encodeURIComponent(article.value.name);
  window.open(`https://social-plugins.line.me/lineit/share?url=${url}&text=${name}`, '_blank');
};
